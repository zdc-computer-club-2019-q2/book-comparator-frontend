                    ...
                    <section className="modal-card-body">
                        <div className="modal-card-details ">
                            <div className="modal-card-header">
                                ...
                                <div className="store-logo" style={{ backgroundImage: `url(${brand})` }} />
                            </div>
                            <small>You will be redirected to external website.</small>
                        </div>
                        <div className="modal-card-buttons">
                            <button className="cta" value="CANCEL" onClick={() => setOpen(false)}>
                                Cancel
                            </button>
                            <button className="cta cancel-cta" onClick={handleContinue}>
                                Continue
                            </button>
                        </div>
                    </section>
...
function Book({ match, history }) {
    ...
    if (response) {
        const { offers, authors = [], title, description, image, categories, recommendation } = response;

        if (!authors.length) {
            return <p>Book does not exist</p>;
        }

        const finalAuthor = authors.join(" & ");
        ...
        return (
            <div id="book">
                ...
                <Redirect brand={brand} open={open} setOpen={setOpen} handleContinue={handleContinue} />
            </div>
        );
    }
